...
class HutangFormScreen extends ConsumerStatefulWidget {
  final HutangEntity? editHutang;

  const HutangFormScreen({this.editHutang, super.key});
...
class _HutangFormScreenState extends ConsumerState<HutangFormScreen> {
  ...
  bool get _isEditing => widget.editHutang != null;

  @override
  void initState() {
    super.initState();
    final h = widget.editHutang;
    if (h != null) {
      _namaController.text = h.namaKreditur;
      _jumlahController.text = h.jumlahAwal.toInt().toString();
      _catatanController.text = h.catatan ?? '';
      _tanggalPinjam = h.tanggalPinjam;
      _tanggalJatuhTempo = h.tanggalJatuhTempo;
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final hutang = HutangEntity(
        id: widget.editHutang?.id ?? uuid.v4(),
        namaKreditur: _namaController.text.trim(),
        jumlahAwal: jumlah,
        sisaHutang: widget.editHutang?.sisaHutang ?? jumlah,
        ...
        status: widget.editHutang?.status ?? 'aktif',
        riwayatPembayaran: widget.editHutang?.riwayatPembayaran ?? [],
        createdAt: widget.editHutang?.createdAt ?? now,
        updatedAt: now,
      );

      final notifier = ref.read(hutangNotifierProvider.notifier);
      if (_isEditing) {
        await notifier.updateHutang(hutang);
      } else {
        await notifier.addHutang(hutang);
      }
  ...
  Widget build(BuildContext context) {
    ...
    final title = _isEditing ? AppStrings.editHutang : AppStrings.tambahHutang;
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        hintText: 'Masukkan nama kreditur',
        ...
        child: Row(
          children: [
            Icon(
              Icons.event_rounded,
              size: 20,
              color: date != null ? AppColors.debt : AppColors.textSecondary,
            ),
            const SizedBox(width: 12),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
